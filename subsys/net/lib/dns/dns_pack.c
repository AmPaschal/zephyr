		      ...
		      enum dns_rr_type *type)
{
	...
	rem_size = dns_msg->msg_size - dns_msg->answer_offset - dname_len;	// Potential vulnerability fix
	if (rem_size < 2 + 2 + 4 + 2) {
		return -EINVAL;
	}
		   ...
		   struct dns_msg_t *dns_msg, uint16_t pos)
{
	...
	*len = 0U;
	...
	while (1) {
		 ...
		 */
		// if (DNS_LABEL_LEN_SIZE + lb_size > size - *len) { // Fix potential vulnerability
